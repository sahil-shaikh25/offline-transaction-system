...
class PayFlowApp:
    ...
    def __init__(self, root):
        ...
        # New user onboarding screen is shown first.
        # The existing Home, Payments, QR, Transactions, Profile, etc.
        # remain unchanged after onboarding is completed.
        self.sidebar.pack_forget()
        self.show_new_user_panel()
    ...
    # ======================================================
    # NEW USER WELCOME / ONBOARDING PANEL
    # ======================================================

    def show_new_user_panel(self):
        """Show a Google Pay-inspired welcome panel for a new user."""
        self.clear_screen()

        # Full-page onboarding area (sidebar stays hidden until Continue).
        page = tk.Frame(self.main, bg=WHITE)
        page.pack(fill="both", expand=True)

        # Top bar
        top = tk.Frame(page, bg=WHITE)
        top.pack(fill="x", padx=55, pady=(25, 10))

        logo = tk.Frame(top, bg=WHITE)
        logo.pack(side="left")

        self.create_label(logo, "G", 36, True, BLUE, WHITE).pack(side="left")
        self.create_label(logo, "Pay", 30, False, "#5F6368", WHITE).pack(side="left")

        self.create_label(top, "⋮", 25, True, GRAY, WHITE).pack(side="right")

        # Welcome heading
        self.create_label(
            page, "Welcome to PayFlow", 27, True, TEXT, WHITE
        ).pack(pady=(15, 4))

        self.create_label(
            page,
            "Make UPI payments, recharges, pay bills & more,",
            13, False, GRAY, WHITE
        ).pack()

        self.create_label(
            page,
            "all in one secure offline transaction app",
            13, False, GRAY, WHITE
        ).pack(pady=(0, 22))

        # Three benefit cards
        benefits = [
            ("₹", "Pay anyone", "Send money to friends\nand merchants"),
            ("✓", "Secure", "Your money and data\nstay protected"),
            ("⚡", "Fast & reliable", "Quick transactions\n24x7"),
        ]

        benefits_frame = tk.Frame(page, bg=WHITE)
        benefits_frame.pack(fill="x", padx=90, pady=(0, 15))
        ...
        for icon, title, desc in benefits:
            ...
            card.pack(side="left", fill="both", expand=True, padx=10, ipadx=8, ipady=10)
            ...
            icon_box.pack(pady=(10, 7))

            self.create_label(card, title, 13, True, TEXT, WHITE).pack()
            self.create_label(
                card, desc, 10, False, GRAY, WHITE
            ).pack(pady=(4, 10))

        # Soft illustration / security banner
        illustration = tk.Frame(
            page,
            bg="#EEF5FF",
            highlightbackground="#D8E7FF",
            highlightthickness=1,
            height=115
        )
        illustration.pack(fill="x", padx=90, pady=(5, 18))
        illustration.pack_propagate(False)

        self.create_label(
            illustration, "◉", 48, True, BLUE, "#EEF5FF"
        ).pack(side="left", padx=(55, 18))

        text_box = tk.Frame(illustration, bg="#EEF5FF")
        text_box.pack(side="left", fill="both", expand=True, pady=20)

        self.create_label(
            text_box, "Simple. Secure. Ready to Pay.", 16, True, TEXT, "#EEF5FF"
        ).pack(anchor="w")

        self.create_label(
            text_box,
            "Set up your mobile number to start using PayFlow.",
            10, False, GRAY, "#EEF5FF"
        ).pack(anchor="w", pady=(5, 0))

        self.create_label(
            illustration, "✓", 28, True, GREEN, "#EEF5FF"
        ...
        form.pack(fill="x", padx=90, pady=(0, 10))

        self.create_label(
            form, "Enter your mobile number", 11, True, TEXT, WHITE
        ).pack(anchor="w", padx=25, pady=(14, 4))

        number_row = tk.Frame(form, bg=WHITE)
        number_row.pack(fill="x", padx=25)

        self.create_label(number_row, "🇮🇳", 18, False, TEXT, WHITE).pack(side="left")
        self.create_label(number_row, "+91", 13, True, TEXT, WHITE).pack(
            side="left", padx=(8, 12)
        )

        # Mobile number input: HARD LIMIT = exactly 10 ASCII digits max.
        # Validation works for typing, paste, cut/replace, and programmatic edits.
        self.new_user_phone_var = tk.StringVar(value="")

        def validate_mobile_key(proposed):
            return proposed == "" or (proposed.isascii() and proposed.isdigit() and len(proposed) <= 10)

        validate_cmd = (self.root.register(validate_mobile_key), "%P")

        self.new_user_phone = tk.Entry(
            number_row,
            textvariable=self.new_user_phone_var,
            font=("Arial", 14),
            fg=TEXT,
            bg=WHITE,
            relief="flat",
        ...
        self.new_user_phone.pack(side="left", fill="x", expand=True, ipady=8)

        def enforce_mobile_limit(event=None):
            # Final safeguard for paste/replace/programmatic insertion.
            current = self.new_user_phone_var.get()
            cleaned = "".join(ch for ch in current if "0" <= ch <= "9")[:10]
            if current != cleaned:
                self.new_user_phone_var.set(cleaned)
            return None

        self.new_user_phone.bind("<KeyRelease>", enforce_mobile_limit)
        self.new_user_phone.bind("<<Paste>>", lambda e: self.root.after_idle(enforce_mobile_limit))

        self.create_label(
            form,
            "This number will be used to securely set up your PayFlow account.",
            9, False, GRAY, WHITE
        ).pack(pady=(2, 13))

        # Consent
        consent = tk.Frame(page, bg=WHITE)
        consent.pack(fill="x", padx=95, pady=(2, 3))

        self.new_user_updates = tk.BooleanVar(value=True)
        tk.Checkbutton(
            consent,
            text="Send me important updates & offers from PayFlow",
            variable=self.new_user_updates,
            font=("Arial", 10),
            fg=TEXT,
            bg=WHITE,
        ...
        self.create_label(
            page,
            "By continuing, you agree to the PayFlow Terms of Service and Privacy Policy.",
            9, False, GRAY, WHITE
        ).pack(pady=(3, 10))
        ...
        self.create_button(
            page,
            "Continue",
            self.complete_new_user_setup,
            BLUE,
            WHITE
        ).pack(fill="x", padx=90, pady=(0, 8), ipady=4)
        ...
        self.new_user_phone.focus_set()

    def validate_mobile_input(self, value):
        """Allow only ASCII numeric digits, maximum 10 digits."""
        return value == "" or (value.isascii() and value.isdigit() and len(value) <= 10)

    def complete_new_user_setup(self):
        """Validate the phone number and open the OTP verification panel."""
        phone = self.new_user_phone.get().strip()

        # Accept only exactly 10 ASCII digits.
        digits = phone

        if not (digits.isascii() and digits.isdigit() and len(digits) == 10):
        ...
        self.user_phone = "+91 " + digits[:5] + " " + digits[5:]
        self.verification_phone = digits

        # Open the second onboarding panel instead of the existing Home screen.
        self.show_otp_panel()

    def show_otp_panel(self):
        """Google Pay-inspired OTP verification screen for a new user."""
        self.clear_screen()
        ...
        self.create_button(
            ...
            self.show_new_user_panel,
            WHITE, TEXT
        ).pack(anchor="w", padx=80, pady=(15, 0))

        self.create_label(
            page, "Verify your mobile number", 27, True, TEXT, WHITE
        ).pack(anchor="w", padx=80, pady=(18, 4))

        self.create_label(
            page,
            f"Enter the 6-digit OTP sent to +91 {self.verification_phone}",
            13, False, GRAY, WHITE
        ...
        card.pack(fill="x", padx=75, pady=(35, 20))

        content = tk.Frame(card, bg=WHITE)
        content.pack(fill="both", expand=True, padx=35, pady=30)

        left = tk.Frame(content, bg=WHITE)
        left.pack(side="left", fill="both", expand=True, padx=(10, 35))

        self.create_label(
            left, "Enter 6-digit OTP", 14, True, TEXT, WHITE
        ).pack(anchor="w", pady=(10, 12))

        # OTP fields
        otp_frame = tk.Frame(left, bg=WHITE)
        otp_frame.pack(anchor="w")

        self.otp_vars = [tk.StringVar() for _ in range(6)]
        self.otp_entries = []

        def otp_validate(proposed):
            return proposed == "" or (proposed.isascii() and proposed.isdigit() and len(proposed) <= 1)

        otp_validate_cmd = (self.root.register(otp_validate), "%P")

        def move_next(index, event=None):
            value = self.otp_vars[index].get()
            if value and index < 5:
                self.otp_entries[index + 1].focus_set()

        def move_previous(index, event=None):
            if event.keysym == "BackSpace" and not self.otp_vars[index].get() and index > 0:
                self.otp_entries[index - 1].focus_set()

        for i, var in enumerate(self.otp_vars):
            entry = tk.Entry(
                otp_frame,
                textvariable=var,
                font=("Arial", 20, "bold"),
                ...
                width=2,
                validate="key",
                validatecommand=otp_validate_cmd
            )
            entry.pack(side="left", padx=(0 if i == 0 else 10))
            entry.bind("<KeyRelease>", lambda e, idx=i: move_next(idx, e))
            entry.bind("<KeyPress>", lambda e, idx=i: move_previous(idx, e))
            self.otp_entries.append(entry)

        self.create_label(
            left, "OTP has been sent to your mobile number", 10, False, GRAY, WHITE
        ).pack(anchor="w", pady=(18, 4))

        timer_row = tk.Frame(left, bg=WHITE)
        timer_row.pack(anchor="w")

        self.otp_timer_label = self.create_label(
            timer_row, "00:60", 11, True, BLUE, WHITE
        )
        self.otp_timer_label.pack(side="left")

        self.create_button(
            timer_row, "Resend OTP", self.resend_otp, WHITE, BLUE
        ).pack(side="left", padx=(20, 0))

        self.create_label(
            left, "🛡  Your number is safe with PayFlow.", 10, False, GRAY, WHITE
        ).pack(anchor="w", pady=(28, 18))

        self.create_button(
            left, "Verify", self.verify_otp, BLUE, WHITE
        ).pack(fill="x", ipady=4)

        # Right side illustration using Tkinter shapes/icons.
        right = tk.Frame(content, bg=WHITE, width=360)
        right.pack(side="right", fill="both", expand=True)
        right.pack_propagate(False)

        art = tk.Frame(right, bg="#F1F6FF")
        art.pack(fill="both", expand=True, padx=20, pady=10)

        self.create_label(
            art, "🔒", 70, True, BLUE, "#F1F6FF"
        ).pack(pady=(35, 5))

        self.create_label(
            art, "Secure verification", 18, True, TEXT, "#F1F6FF"
        ).pack()

        self.create_label(
            art,
            "Your mobile number is protected\nand used only to verify your account.",
            10, False, GRAY, "#F1F6FF"
        ).pack(pady=10)

        self.create_label(
            art, "✓  Protected\n✓  Private\n✓  Trusted",
            12, True, GREEN, "#F1F6FF"
        ).pack(pady=18)

        # Security highlights below the card
        self.create_label(
            page, "Your security is our priority", 12, True, GRAY, WHITE
        ).pack(pady=(0, 8))

        highlight = tk.Frame(page, bg=WHITE)
        highlight.pack(fill="x", padx=75)

        items = [
            ("✓", "100% Secure", "Your payments are protected"),
            ("♙", "Private", "Your data stays private"),
            ("₹", "Trusted", "Secure UPI payments"),
            ("🇮🇳", "Made in India", "Secure payments for everyone")
        ]
        ...
        for icon, title, desc in items:
            ...
            self.create_label(box, icon, 22, True, BLUE, WHITE).pack(pady=(8, 2))
            self.create_label(box, title, 10, True, TEXT, WHITE).pack()
            self.create_label(box, desc, 8, False, GRAY, WHITE).pack(pady=(2, 8))

        self.root.after(100, lambda: self.otp_entries[0].focus_set())

        self.otp_seconds = 60
        self.update_otp_timer()

    def update_otp_timer(self):
        """Update the OTP countdown once per second."""
        if not hasattr(self, "otp_timer_label") or not self.otp_timer_label.winfo_exists():
            return

        if self.otp_seconds > 0:
            self.otp_timer_label.config(text=f"00:{self.otp_seconds:02d}")
            self.otp_seconds -= 1
            self.otp_timer_job = self.root.after(1000, self.update_otp_timer)
        else:
            self.otp_timer_label.config(text="00:00")

    def resend_otp(self):
        """Reset the OTP timer and clear the OTP fields for demo purposes."""
        if hasattr(self, "otp_timer_job"):
            try:
                self.root.after_cancel(self.otp_timer_job)
            except Exception:
                pass

        for var in self.otp_vars:
            var.set("")

        self.otp_seconds = 60
        self.create_otp_demo_value = True
        self.update_otp_timer()
        self.otp_entries[0].focus_set()
        messagebox.showinfo(
            "OTP Resent",
            f"A new 6-digit OTP has been sent to +91 {self.verification_phone}."
        )

    def verify_otp(self):
        """Verify the 6-digit demo OTP and then open the existing Home screen."""
        ...
        # Demo/offline application behavior: any valid 6-digit OTP is accepted.
        # No real SMS service is connected to this desktop app.
        if hasattr(self, "otp_timer_job"):
            try:
                self.root.after_cancel(self.otp_timer_job)
            except Exception:
        ...
        messagebox.showinfo("Verified", "Mobile number verified successfully!")

        # Restore the existing application layout exactly as before.
        self.sidebar.pack(side="left", fill="y")
        self.show_home()
